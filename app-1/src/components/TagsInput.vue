<template>
  ...
  <div class="tags-input-wrapper">
    ...
    <div v-if="(!tags.length)">
      You have no ant tags. Enter it in the input.
    </div> -->
    <!-- <div v-for="(tag, index) in tags">{{ index + ' : ' + tag }}</div> -->
    <span class="tag-item" v-for="(tag, index) in tags" :key="index">
      {{ tag }}
      <a class="remove-tag" @click.prevent="removeTag(index)" href="javascript:;">&times;</a>
    </span>
    <!-- <input type="text" v-bind:value="newTag" v-on:keydown.enter="tags.push($event.target.value)" /> -->
